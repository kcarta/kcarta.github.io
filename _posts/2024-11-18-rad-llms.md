...
RAD had its heyday in the 90s, with millions of developers using toolkits like [Visual Basic](https://en.wikipedia.org/wiki/Visual_Basic_(classic)) to produce working software that could quickly get in the hands of its users. 
...
In the world of business software, there are impediments that no development practice or single technology can overcome:
...
- Bad product management where user adoption is either not considered or not properly designed for.
